...
class _RoomInfoPageState extends State<RoomInfoPage> {
  ...
  final CollectionReference _managerPosts = FirebaseFirestore.instance.collection("Manager Posts");
//Access
  ...
  Future<void> checkManagerAccess()async{
    final managerPostsQuery = _managerPosts.where("ManagerEmail", isEqualTo: currentUser.email );
              ...
              TextButton(
                  onPressed: (){
                    if (_managementTitle.text.isNotEmpty){
                      FirebaseFirestore.instance.collection("Manager Posts").doc(currentUser.email).collection("Management").add({
                 ...
                 TextButton(
                     onPressed: (){
                       if (AddressTextController.text.isNotEmpty){
                         FirebaseFirestore.instance.collection("Manager Posts").doc(currentUser.email).collection("Info").add({
                                ...
                                children: [
                                  ...
                                  ListView.builder(
                                    ...
                                    itemBuilder: (context, index){
                                      ...
                                      final title = data.get("Title").toString();
                                      final cost = data.get("Cost").toString();
                                      return Center(
                                        child: Padding(
                                          ...
                                          child: Row(
                                            mainAxisAlignment: MainAxisAlignment.center,
                                            children: [
                                              SizedBox(width: 80, height: 50,child: Text(title, style: const TextStyle(fontSize: 24),),),
                                              SizedBox(width: 80, height: 50,child: Text(cost, style: const TextStyle(fontSize: 24),),),
                                            ],
                                          ),
                                        ),
                                      );
