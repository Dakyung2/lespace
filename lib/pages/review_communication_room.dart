...
class ReviewCommunicationRoom extends StatefulWidget {
  ...
  const ReviewCommunicationRoom({Key? key, required this.address})
      : super(key: key);

  @override
  State<ReviewCommunicationRoom> createState() =>
      _ReviewCommunicationRoomState();
}
...
class _ReviewCommunicationRoomState extends State<ReviewCommunicationRoom> {
  ...
  Future<void> _getOrCreateDocument() async {
    ...
    if (!documentSnapshot.exists) {
      ...
      await collection
          .doc(widget.address)
          .set({"Address": widget.address}); //return newDoc;
    }
  }
  ...
  Future<bool> _getAllDocs() async {
    ...
    for (QueryDocumentSnapshot<Map<String, dynamic>> document
      ...
      isCurrentUserIncluded =
          docData["UserEmail"]?.contains(currentUserEmail) ?? false;
      if (isCurrentUserIncluded) {
        break;
      }
    }
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Center(child: Text(" ${widget.address} 경험자분들과의 Q&A")),
            const SizedBox(
              height: 10,
            ),
            Expanded(
                      ...
                      return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            DateTime commentDateTime =
                                DateTime.parse(commentTime.toDate().toString());
                            String commentFormattedDateTime =
                                DateFormat('yy/MM/dd').format(commentDateTime);
                            return Card(
                              ...
                              child: Padding(
                                ...
                                child: ListTile(
                                  ...
                                  subtitle: Row(
                                    ...
                                    children: [
                                      if (commentdocumentSnapshot[
                                              "UserStatus"] !=
                                          null)
                                        Text(
                                          commentdocumentSnapshot["UserStatus"]
                                              .toString(),
